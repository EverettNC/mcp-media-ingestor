    ...
    def _crawl_loop(self):
        ...
        while self.running:
            ...
            for url in TOPIC_SOURCES.get(topic, []):
                try:
                    r = requests.get(url, headers={"User-Agent": "alphavoxBot/1.0"}, timeout=10)
                    ...
                    for p in soup.find_all("p")[:5]:
                        ...
                        if len(text.split()) < 6:
                            continue
                        fact_manager.add_fact(text, source=url, topics=[topic], confidence=0.85)
...
__all__ = ['get_knowledge_engine', 'KnowledgeGraph', 'FactManager', 'WebCrawler', 'KnowledgeEngine']
